...
@auto_docstring(checkpoint="PoSTMEDIA/Rosetta-7B-Instruct")
@strict
class RosettaConfig(PreTrainedConfig):
    r"""
    Example:

    ```python
    >>> from transformers import RosettaModel, RosettaConfig

    >>> # Initializing a Rosetta 7B style configuration
    >>> configuration = RosettaConfig()

    >>> # Initializing a model from the Rosetta 7B style configuration
    >>> model = RosettaModel(configuration)
