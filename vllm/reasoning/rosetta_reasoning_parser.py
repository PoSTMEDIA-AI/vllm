...
# Copyright 2026 PoSTMEDIA. All rights reserved.
# This file has been modified from its original version by PoSTMEDIA.
...
class RosettaReasoningState(enum.Enum):
    REASONING = 1
    CONTENT = 2
...
class RosettaReasoningBuffer:
    ...
    def process_buffer(self) -> DeltaMessage | None:
        ...
        if end_think_idx >= 0:
            self.state = RosettaReasoningState.CONTENT
            pretext, self.buffer = (
        ...
        if self.state == RosettaReasoningState.REASONING:
        ...
        if self.state == RosettaReasoningState.CONTENT:
...
class RosettaReasoningParser(ReasoningParser):
    """
    Reasoning parser for Rosetta model

    RosettaReasoningParser

    This class implements a reasoning parser specifically designed for the
    Rosetta family of models. Rosetta models do not use special tokens to
    ...
    Key Features:
        ...
        - For stream process, it uses a buffer to accumulate delta text,
          ...
          or ends.
        - For reliability, some Rosetta models may hardcode the first
          `<think>` token is the input text (similar to other
          reasoning models). To support such variants, the
    ...
    def __init__(self, tokenizer: "TokenizerLike", *args, **kwargs):
        ...
        self.buffer = RosettaReasoningBuffer(
    ...
    def extract_content_ids(self, input_ids: list[int]) -> list[int]:
        # for Rosetta streaming reason parsing, the stream parse
